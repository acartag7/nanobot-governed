...
class AgentLoop:
    ...
    def __init__(
        ...
        self._edictum_config = edictum_config
    ...
    async def _connect_edictum(self) -> None:
        """Set up edictum governance if configured."""
        if not self._edictum_config:
            return

        server_url = self._edictum_config.get("server_url")
        api_key = self._edictum_config.get("api_key")

        if server_url and api_key:
            from nanobot.agent.governance import create_governed_registry_from_server

            self.tools = await create_governed_registry_from_server(
                self.tools,
